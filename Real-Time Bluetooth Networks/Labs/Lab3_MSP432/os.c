...
struct ptcb{
  int32_t period;      // period of the task
	int32_t counter;     // task counter
  void (*task)(void);  // pointer to the task
};
typedef struct ptcb ptcbType;
ptcbType ptcbs[NUMPERIODIC];
...
// ****IMPLEMENT THIS****
  static int16_t ptasknum;
	
  int32_t status;
  status = StartCritical();
	if(ptasknum < NUMPERIODIC) {
		ptcbs[ptasknum].period = period;
		ptcbs[ptasknum].task   = thread;
		ptasknum++;
		EndCritical(status);
    return 1;
	}
...
// **RUN PERIODIC THREADS, DECREMENT SLEEP COUNTERS
	for (int32_t i = 0; i < NUMPERIODIC; i++) {
		if(ptcbs[i].counter++ >= (ptcbs[i].period - 1)) {
			ptcbs[i].counter = 0;
			(*ptcbs[i].task)();
		}
	}
...
//***IMPLEMENT THIS***
	...
	*semaPt += 1;
	if (*semaPt <= 0) {
		pt = RunPt->next;               // search for a thread blocked for this semaphore
    while(pt->blocked != semaPt) {
      pt = pt->next;
		}			
		pt->blocked = 0;                // wake this task
	}
	EnableInterrupts();
}
