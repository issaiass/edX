...
void ByteCpy(const uint8_t *msgsrc, uint8_t *msgdst, uint8_t size);
...
//****You implement this function as part of Lab 6*****
  ...
  framesize = AP_GetSize(msg) + 5;
  for(fcs = 0, i = 1; i < framesize; i++) {
		fcs = fcs^(*(msg + i));
	}
  *(msg + i) = fcs;
}
...
const uint8_t GetStatus[] =   {SOF,0x00,0x00,0x55,0x06};
const uint8_t GetVersion[] =  {SOF,0x00,0x00,0x35,0x03,0x36};  
const uint8_t AddService[8] = {
																SOF,3,0x00,       // length = 3
...
const uint8_t AddNotifyCharDescriptor[] = {   
																	SOF,            // Start of Frame
																	' ',0x00,       // Lenght
...
const uint8_t GATTSetDeviceName[] = {   
																	SOF,
																	' ',0x00,       // length
...
const  uint8_t SetAdvertisementMsg[] = {   
																				SOF,
																				' ',0x00,       // length = 11
...
const uint8_t SetAdvertisementData[] = {   
																			SOF,
																			' ',0x00,       // length
																			0x55,0x43,      // SNP Set Advertisement Data
...
const uint8_t StartAdvertisement[] = {   
																			SOF,
																			' ',0x00,       // length = 14
...
//****You implement this function as part of Lab 6*****
  framesize = sizeof(GetStatus);	
	ByteCpy(GetStatus, msg, framesize);
	SetFCS(msg);
}
//*************Lab6_GetStatus**************
...
//****You implement this function as part of Lab 6*****
  framesize = sizeof(GetVersion);	
  ByteCpy(GetVersion, msg, framesize);
	SetFCS(msg);
}
//*************Lab6_GetVersion**************
...
//****You implement this function as part of Lab 6*****
  framesize = sizeof(AddService);	
	ByteCpy(AddService, msg, framesize);
	msg[6] = uuid;
	msg[7] = uuid>>8;
	SetFCS(msg);
}
...
//****You implement this function as part of Lab 6*****
  framesize = sizeof(Register);	
  ByteCpy(Register, msg, framesize);	
	SetFCS(msg);
}
...
//****You implement this function as part of Lab 6*****
	framesize = sizeof(AddCharValue);
  ByteCpy(AddCharValue, msg, framesize);	
	msg[5] = permission;
	msg[6] = properties;
	msg[11] = uuid;
	msg[12] = uuid>>8;
...
const uint8_t AddCharDescriptor[] = {   
...
//FCS                // FCS (calculated by AP_SendMessageResponse)
  };   
...
//****You implement this function as part of Lab 6*****
  strsize     = strlen(name); 
  payloadsize = sizeof(AddCharDescriptor) + strsize + 1 - 5;//6 + strsize + 1;
	framesize   = sizeof(AddCharDescriptor) + strsize + 1;
  ByteCpy(AddCharDescriptor, msg, framesize);
  StrIns(msg, name, 11, strsize);
	msg[1] = payloadsize;
	msg[7] = strsize + 1; // non-counting the zero
	msg[9] = strsize + 1;
	SetFCS(msg);
}
...
//****You implement this function as part of Lab 6*****
  strsize     = strlen(name); 
  payloadsize = sizeof(AddNotifyCharDescriptor) + strsize + 1 - 5;//7 + strsize + 1;
	framesize   = sizeof(AddNotifyCharDescriptor) + strsize + 1;
  ByteCpy(AddNotifyCharDescriptor, msg, framesize);	
  StrIns(msg, name, 12, strsize);
  msg[1]  = payloadsize;
  msg[8]  = strsize + 1;
  msg[10] = strsize + 1;
	SetFCS(msg);
...
//****You implement this function as part of Lab 6*****
  strsize     = strlen(name); 
  payloadsize = sizeof(GATTSetDeviceName) + strsize - 5;//3 + strsize;
	framesize   = sizeof(GATTSetDeviceName) + strsize;
  ByteCpy(GATTSetDeviceName, msg, framesize);
	ByteIns(msg, name, 8, strsize);
	msg[1] = payloadsize;
	SetFCS(msg);
}
...
// Key state=0
//****You implement this function as part of Lab 6*****
  framesize = sizeof(SetAdvertisementMsg);	
  ByteCpy(SetAdvertisementMsg, msg, framesize);
	msg[1] = sizeof(SetAdvertisementMsg) - 5;
	SetFCS(msg);
}
...
//****You implement this function as part of Lab 6*****
  ...
  strsize     = strlen(name); 
  payloadsize = sizeof(SetAdvertisementData) + strsize - 5;
	framesize   = sizeof(SetAdvertisementData) + strsize;
  ByteCpy(SetAdvertisementData, msg, framesize);
  ByteCpy(&SetAdvertisementData[8], buff, 8);
	ByteIns(msg, name, 8, strsize);
...
//****You implement this function as part of Lab 6*****
  framesize = sizeof(StartAdvertisement);	
  ByteCpy(StartAdvertisement, msg, framesize);
	msg[1] = sizeof(StartAdvertisement) - 5;
	msg[8] = interval;
  SetFCS(msg);
}
