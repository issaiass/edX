...
//	
//			
//	ROMAddr = EDISK_ADDR_MIN + 512*sector;
//	if (ROMAddr < EDISK_ADDR_MAX){
//	  for (i = 0; i < 512; i++){
//		  *(buff + i) = (*((volatile uint32_t *)(ROMAddr + i)));
//	  }
//	  return RES_OK;
//	}
//	return RES_PARERR;}
  uint16_t i;
  uint32_t start_rom_address; 
  uint8_t *rom_address_ptr;
			
			
  start_rom_address = EDISK_ADDR_MIN + 512*sector;	
  rom_address_ptr=(uint8_t *)start_rom_address;
  if(start_rom_address > EDISK_ADDR_MAX) {
		return RES_PARERR;
	}
  for (i = 0; i < 512; i++) {
		*buff = *rom_address_ptr;
     rom_address_ptr++;
		 buff++;
	}
  return RES_OK;
...
//		return RES_PARERR;
//	}		
//  Flash_WriteArray((uint32_t *)buff, ROMAddr, 128); // 512/4 = 128
//  return RES_OK;
  uint32_t start_rom_address;
  uint32_t *ram_address_ptr;

			
  ram_address_ptr = (uint32_t *)buff;
  start_rom_address = EDISK_ADDR_MIN + 512*sector;
  if (start_rom_address > EDISK_ADDR_MAX) {
    return RES_PARERR;
	} else {
		if(Flash_WriteArray(ram_address_ptr, start_rom_address, 128)!= 128) {
			return RES_ERROR;
		}
  }
  return RES_OK;
...
//	address = EDISK_ADDR_MIN; //start from the beginning
//	while (address < EDISK_ADDR_MAX) {
//    if (Flash_Erase(address) != RES_OK) {
//		  return RES_ERROR;
//	  }
